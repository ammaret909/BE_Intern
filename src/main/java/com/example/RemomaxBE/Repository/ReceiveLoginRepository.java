...
public interface ReceiveLoginRepository extends JpaRepository<ReceiveLoginModel,String> {
    ...
    @Query(value = "SELECT * FROM ulogio WHERE rccu = :rccUser AND in_or_out = '0'",nativeQuery = true)
    List<ReceiveLoginModel> findAllLoginByRccUserLogin(String rccUser);
    @Query(value = "SELECT * FROM ulogio WHERE rcc_login = :rccLogin AND in_or_out = '1'",nativeQuery = true)
    ReceiveLoginModel findLogoutByRcc(String rccLogin);
